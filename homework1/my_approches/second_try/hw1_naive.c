...
generate a segmentation fault. To avoid segmentation faults, declare large arrays as global 
variables.*/
int naive(int, int[]);
...
int main(){
    ...
    for(int i = 0; i < s_len; i++){//initalize idx_arr
        idx_arr[i] = -1;
    }
    int cnt = naive(s_len, idx_arr);
    printf("%d\n", cnt);
...
int naive(int s_len, int idx_arr[]){
    int cnt = 0;
    int s_idx = 0;
    int head_m = 0;
    int j = 0;
